# Ori Mushi

::: rules
<img class="size10" alt="" src="cc-imgs/japan_flag_cool_wallpaper_by_Ir_IA_cc-by-nc_noBg.jpg">

<br>

Un jeu de rôle _hopepunk_ dans un univers de _fantasy_ inspiré du japon médiéval,
où les joueurs incarnent des _komusō_, ayant fait vœu d'aider la population,
et où des créatures nommées _mushis_ ont donné naissance à la magie.

**Inspirations**: Mushishi, Le château dans le ciel, Shadow of the Colossus, Avatar le maître de l'air, Naruto, Princesse Mononoké, Le Voyage de Chihiro, Full Metal Alchemist, Zelda...

Retrouvez les articles de mon blog à propos du développement du jeu :
<br>[tag #ori-mushi @ chezsoi.org](https://chezsoi.org/lucas/blog/tag/ori-mushi.html)

<!-- Pitch: -->
## Pourquoi Ori Mushi devrait vous plaire ?
Voici les atouts principaux de ce jeu :

* des règles **simples**, accessibles à tout âge
* les joueurs participent à **créer des éléments de l'univers**
* un **écosystème magique** et une **histoire secrète** sont au cœur de l'univers, une invitation pour les joueurs à trouver la clef de nombreux **mystères**
* place les joueurs au cœur d'une histoire **épique et émouvante**, une "campagne" captivante, qui peut se jouer en "table ouverte"
* un jeu où **se battre est rarement la solution**, et où les problèmes se résolvent par l'entraide et l'imagination

Mais aussi :
* un **carnet d'aventures** rédigé petit à petit par les joueurs
* une **création des personnages rapide** : 10min par personnage environ
* **la progression des personnages amène la progression de l'histoire** <!-- le passage de palier de Voie déclenche les actes 2 & 3 -->
* un jeu de rôle adapté pour des enfants ou des ados, et pour faire découvrir le jeu de rôle

<!--
Ambitions personnelles de game design :
* essayer de rester SIMPLE & ACCESSIBLE
* les personnages et l'univers **évoluent significativement** durant la campagne
* les FPs intègrent des **mécaniques faisant évoluer narrativement** chaque personnage, avec des éléments **uniques pour chaque archétype**
* un jeu **_light_ en terme de contenu textuel** décrivant l'univers, mais intégrant à la place des phases de **narrativistes**. L'idée est d'inclure le minimum de _lore_ possible, uniquement le strict nécessaire, mais que les joueurs puissent contribuer à définir des pans de l'univers durant certaines phases de jeu
-->

## Quelques illustrations
Réalisées par [Elliot Jolivet](https://illutensei.com/) spécialement pour _Ori Mushi_.
Elliot sera l'illustrateur du jeu, une fois la phase de conception terminée :

<div class="side-by-side">
  <img alt="Bao, un personnage du jeu de rôle Ori Mushi" src="https://chezsoi.org/lucas/jdr/OriMushi/illus-tensei/PJ-Bao.jpg" >
  <img alt="Boo, un personnage du jeu de rôle Ori Mushi" src="https://chezsoi.org/lucas/jdr/OriMushi/illus-tensei/PJ-Boo.jpg" >
</div>

<div class="side-by-side">
  <img alt="Eishun, un personnage du jeu de rôle Ori Mushi" src="https://chezsoi.org/lucas/jdr/OriMushi/illus-tensei/PJ-Eishun.jpg" >
  <img alt="Kaito, un personnage du jeu de rôle Ori Mushi" src="https://chezsoi.org/lucas/jdr/OriMushi/illus-tensei/PJ-Kaito.jpg" >
</div>

<div class="side-by-side">
  <img alt="Yasuo, un personnage du jeu de rôle Ori Mushi" src="https://chezsoi.org/lucas/jdr/OriMushi/illus-tensei/PJ-Yasuo.jpg" >
</div>

:::
