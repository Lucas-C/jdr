## Version v0.5 - Partie du 28 février 2026
* rédaction d'un [pitch](./Pitch.pdf) du jeu, affiché sur la page d'accueil web
* compétences :
    + désormais il y a 2 niveaux de maîtrise
    + 2 compétences supplémentaires
    + renommage de plusieurs compétences (_cf._ dossier `old/`)
* révision de la Feuille de Personnage, et du format de la Feuille de Voie, désormais découpée en 3 paliers
* table ouverte :
    + création d'un [Livret d'Aventures](character-sheets/OriMushi-LivretDAventures.pdf)
    + mécaniques narratives de début de partie : _"Pendant ce temps"_ introduit
* développement de l'intrigue, de la 1ère ellipse et des nouvelles Voies accessibles
* univers :
    + Prêtres du Vide (= institution des komusō)

## Version v0.4 - Partie du 5 juillet 2025
Renommages :
* Hautes Terres -> Hauts Plateaux
* Mythos -> Mystika
* naikokujin -> forestiers
* nanbanjin -> bateliers
* Guerre de l'oubli :
    + Peuple oublié -> Peuple éteint
* Jutsu -> Vocables :
    + une technique de jutsu -> un Verset
